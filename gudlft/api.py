...
def update_number_of_places(db, placesRequired, competition, club):
    numberOfPlacesUpdated = int(
        competition['numberOfPlaces'])-int(placesRequired)
    ...
    if booking:
        numberOfPlaceAvailable = 12 - \
            sum([book['nbPlaces'] for book in booking]) - int(placesRequired)

    if numberOfPointsUpdated > 0 and numberOfPlacesUpdated > 0 and numberOfPlaceAvailable > 0:
        db.execute(
            "INSERT INTO booking (id_competition, id_club, nbPlaces) VALUES (?,?,?)", (int(
                competition['id']), int(club['id']), int(placesRequired))
        )
        db.execute(
            'UPDATE competitions SET numberOfPlaces=? WHERE id=?', (int(
                numberOfPlacesUpdated), int(competition['id']),)
        )
        db.execute(
            'UPDATE clubs SET points=? WHERE id=?', (int(
                numberOfPointsUpdated), int(club['id']),)
        )
        db.commit()
        flash(
...
def get_booking(db, id_competition=None, id_club=None):
    return db.execute(
        "SELECT * FROM booking WHERE id_competition=? AND id_club=?", (int(
            id_competition), int(id_club))
    ).fetchall()
